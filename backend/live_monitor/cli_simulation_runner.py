"""
cli_simulation_runner.py
NETRIQ Live Monitor Command Line Interface & Simulation Runner.
Provides CLI execution for synthetic flow simulation testing.
"""
...
def check_is_internal_flow(src_ip: str, dst_ip: str) -> bool:
    """Checks if flow originates from an internal private IP address."""
    return is_internal_ip(src_ip)


class LiveMonitorRunner:
    """
    Master pipeline orchestrating the CLI execution chain:
    packet_sniffer -> flow_builder -> feature_extractor -> NetriqEngine
    """
    ...
    def process_packet(self, pkt_info: dict, enforce: bool = False) -> list:
        ...
        for flow in completed_flows:
            ...
            is_internal = is_internal_ip(eval_ip)
            
            # 2. Evaluate features through NetriqEngine
    ...
    def run_simulation(self, count: int = 5, delay_sec: float = 0.5):
        ...
        external_ips = ["185.220.101.5", "198.51.100.12", "198.51.100.89", "45.33.32.156"]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NETRIQ Live Monitor CLI Simulation Runner")
    ...
    if args.mode == "simulate":
        ...
        print("Use API endpoint POST /api/v1/monitoring/start for production monitoring.")
